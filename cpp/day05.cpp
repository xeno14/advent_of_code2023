// g++ -O3 -fopenmp day05.cpp  && time ./a.out
...
struct Map {
    ...
    Map(const std::vector<Range> &r) : ranges(r) {
    }

    ull operator()(ull x) const {
        for (auto [dst, src, rng] : ranges) {
            if (src <= x && x < src + rng) {
                return dst + x - src;
            }
        }
...
ull part1(const std::vector<ull> &seeds,
          const std::vector<std::vector<Range>> &maps) {
    ull ans = UINT64_MAX;

    std::vector<Map> map_funcs;
    for (const auto &ranges : maps)
        map_funcs.emplace_back(ranges);

    for (auto seed : seeds) {
        auto x = seed;
        for (auto &m : map_funcs) {
            x = m(x);
        }
        ans = std::min(ans, x);
    }
...
ull part2(const std::vector<ull> &seeds,
          const std::vector<std::vector<Range>> &maps) {
    std::vector<Map> map_funcs;
    for (const auto &ranges : maps)
        map_funcs.emplace_back(ranges);

    std::vector<ull> mins(seeds.size() / 2);
    int size = mins.size();

#pragma omp parallel for
    for (int i = 0; i < size; ++i) {
        ull seed_start = seeds[i * 2];
        ull seed_size = seeds[i * 2 + 1];

        printf("thread = %d, i = %2d seed=%llu size=%llu\n",
               omp_get_thread_num(), i, seed_start, seed_size);

        ull dst = UINT64_MAX;
        for (ull seed = seed_start; seed < seed_start + seed_size; ++seed) {
            auto x = seed;
            for (const auto &m : map_funcs) {
                x = m(x);
            }
            dst = std::min(dst, x);
        }
        mins[i] = dst;
        printf("thread = %d, i = %2d dst=%llu\n", omp_get_thread_num(), i, dst);
    }
...
void example() {
    std::vector<ull> seeds = {79, 14, 55, 13};
    ...
    std::cout << "part1 example=" << part1(seeds, maps) << std::endl;
    std::cout << "part2 example=" << part2(seeds, maps) << std::endl;
}

void solve() {
    std::vector<ull> seeds = {1310704671, 312415190, 1034820096, 106131293,
                              682397438,  30365957,  2858337556, 1183890307,
                              665754577,  13162298,  2687187253, 74991378,
                              1782124901, 3190497,   208902075,  226221606,
                              4116455504, 87808390,  2403629707, 66592398};
    std::vector<std::vector<Range>> maps{
        {
    ...
    std::cout << "part1 = " << part1(seeds, maps) << std::endl;
    std::cout << "part2 = " << part2(seeds, maps) << std::endl;
}
